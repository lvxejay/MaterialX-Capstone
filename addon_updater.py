...
class Singleton_updater(object):
	...
	def get_raw(self, url):
		...
		try:
			...
			self._update_ready = None
			if self._verbose: print(e, url, request)
	...
	def past_interval_timestamp(self):
		...
		if "last_check" not in self._json or self._json["last_check"] == "":
			...
			if delta.total_seconds() > 0:
				...
				if self._verbose:
					print("{} Updater: Determined it's not yet time to check for updates"
						  .format(self._addon))
				return False
	...
	def set_updater_json(self):
		...
		if os.path.isfile(jpath):
			with open(jpath) as data_file:
				...
				if self._verbose: print("{} Updater: Read in json settings from file"
										.format(self._addon))
		else:
			# set data structure
			self._json = {
	...
	def save_updater_json(self):
		...
		outf.close()
		if self._verbose:
			print(self._addon+": Wrote out updater json settings to file, "
							  "with the contents:")
			print(self._json)
	...
	def start_async_check_update(self, now=False, callback=None):
		if self._async_checking == True:
			return
		if self._verbose: print("{} updater: Starting background checking thread"
								.format(self._addon))
	...
	def async_check_update(self, now, callback=None):
		...
		if self._verbose: print("{} BG thread: Checking for update now in background"
								.format(self._addon))
		...
		if self._verbose:
			print("{} BG thread: Finished checking for update, doing callback"
				  .format(self._addon))
