...
updater.addon = "onelvxe_material_pipeline"
...
class addon_updater_check_now(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        settings = context.user_preferences.addons["material_pipeline"].preferences
...
def post_update_callback(res=None):
    ...
    if res==None:
        ...
        if updater.verbose: print("{} updater: Running post update callback"
                                  .format(updater.addon))
...
def check_for_update_background():
    ...
    # apply the UI settings
    addon_prefs = bpy.context.user_preferences.addons.get('material_pipeline', None)
    if not addon_prefs:
        return
...
def register_updater(bl_info):
    ...
    updater.private_token = "dMYaNyLehU7w-Fdexc9f"

    # choose your own username
    updater.user = "onelvxe_guest"

    # choose your own repository, must match github name
    updater.repo = "4021194"

    #updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but reocmmended to set
    updater.website = "https://gitlab.com/lvxejay/material_pipeline"
    ...
    # Alternate example
    # updater.backup_ignore_patterns = [".git", "__pycache__", "*.bat", ".gitignore", "
    # *.exe"]
